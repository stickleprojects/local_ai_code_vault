...
**Created:** 2026-05-16 · **Revised:** 2026-05-19 (Phases 1–4 all merged; status corrected — was stale at "Next: Phase 2.1"). Prior revisions: 2026-05-17 (Phase 1 complete: 1.1–1.4 merged; **Phase 2 resequenced** — scripts+error-handling first, thin SKILL.md last); 2026-05-16 (decomposed stack, ephemeral indexer, Option B, script-based skill, languages C#/Py/JS/TS, introspection first-class; **B-1 RESOLVED**: nomic-embed-code Q4_K_M via llama.cpp `--pooling last`, dim 3584)
**Status:** ✅ **Phases 1–4 complete and merged** (1 API/registry/indexer/query/stack, 2 scripts+thin skill, 3 testing+docs, 4.2 lightweight release; **4.1 & 4.3 deferred** — see Phase 4 rationale). Post-merge hardening since: skill made repo-agnostic & prompt-quiet, indexer obj/bin prune (#19), `-Rebuild` (#20). ▶ **In flight:** PR #21 — `install-skill.ps1` permission-hook installer (opt-in bypass of the per-call approval prompt, fail-closed, AV-citizen, `-Remove` revokes). ▶ **Remaining (user, on the NVIDIA box):** restart Claude Code → `-Rebuild` re-index → Phase 3.2 ~30-min manual multi-project validation → one-time live VS Code/Copilot MCP adapter smoke (handshake + `mcp.servers` shape not covered by tests).
